...
The bootstrap is deterministic from the declared bootstrap seed. Test labels are used only for final scoring and bootstrap uncertainty estimation, never checkpoint selection or tuning. Generated CSV files belong under `results/` and remain ignored by Git.
